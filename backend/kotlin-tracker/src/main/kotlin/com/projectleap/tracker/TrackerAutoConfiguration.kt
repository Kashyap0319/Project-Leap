...
class TrackerAutoConfiguration {
    ...
    @Bean
    fun trackingJwtService(props: MonitoringProperties) = JwtService(props.jwtSecret)

    @Bean
    fun rateLimitConfigFetcher() = RateLimitConfigFetcher()

    @Bean
    fun trackingRateLimiter(props: MonitoringProperties, fetcher: RateLimitConfigFetcher, jwt: JwtService): RateLimiter {
    ...
    fun logBatcher(props: MonitoringProperties, rateLimiter: RateLimiter, jwt: JwtService): LogBatcher =
        LogBatcher(
